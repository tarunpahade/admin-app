...
function topProducts(data) {
  ...
  function maap(y) {
    y.map((z) => {
      ...
      const retiontiom = x.map(a => a.quantity)
      retain.push(retiontiom.length)
      console.log(retiontiom);
      count.push(add)
    })
  }
...
async function categoryWiseSale(data) {
  ...
  data.map((y) => {
    ...
    y.orderedFood.map((o) => {
      ...
      const mappeddata = data2.filter((u) => {
        if (/\s/g.test(u.item) === true) {
